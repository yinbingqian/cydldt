...
import com.lnpdit.chatuidemo.db.InviteMessgeDao;
import com.lnpdit.chatuidemo.db.UserDao;
import com.lnpdit.chatuidemo.domain.InviteMessage;
import com.lnpdit.chatuidemo.domain.InviteMessage.InviteMesageStatus;
...
//		chatHistoryFragment = new ChatHistoryFragment();
		...
		// 添加显示第一个fragment
		getSupportFragmentManager().beginTransaction().add(R.id.fragment_container, indexFragment).add(R.id.fragment_container, chatHistoryFragment)
		.show(indexFragment).hide(contactListFragment).hide(chatHistoryFragment)
				.commit();
...
//		registerReceiver(offlineMessageReceiver, offlineMessageIntentFilter);
	...
	private void initView() {
		...
		mTabs[3] = (Button) findViewById(R.id.btn_index);
		// 把第一个tab设为选中状态
...
//		}
	...
	public int getUnreadAddressCountTotal() {
		...
		if (CydlApplication.getInstance().getContactList().get(Constant.NEW_FRIENDS_USERNAME) != null)
			unreadAddressCountTotal = CydlApplication.getInstance().getContactList().get(Constant.NEW_FRIENDS_USERNAME)
					.getUnreadMsgCount();
...
//	};
	...
	private class MyContactListener implements EMContactListener {
		...
		public void onContactAdded(List<String> usernameList) {
			// 保存增加的联系人
			Map<String, User> localUsers = CydlApplication.getInstance().getContactList();
